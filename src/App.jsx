 ...
 *          [차트탭 종합판정 메시지 통일]
 *                 "시점+추세 동반" → "돌파+강세 동반"
 *                 "시점만 떴고 추세 약화" → "돌파만 발생, 강세 약함"
 *                 "추세 견고, 시점 미흡" → "강세 견고, 돌파 미흡"
 *          [실험실 컴포넌트 그룹 라벨 통일]
 *                 group: "시점" → "돌파" (4개: ST flip / MACD골든 / 구름돌파 / 스퀴즈오프)
 *                 group: "추세" → "강세" (4개: ST 3/3 / MACD>Signal / MA20위 / 3봉전위)
 *                 그대로: "거래량" (2개), "전환" (4개)
 *          [실험실 시그널 조합 카드 라벨]
 *                 "🚀 진입적기" → "🌟 둘 다 충족" (이름 어색하던 것 정리)
 *          [집중탭 카드 정리]
 *                 "🎯 진입적기 순위" 카드 제거 (💪 그냥 강함과 중복)
 *                 → 기본 뷰: "🚀 오늘의 돌파 감지"만 남음
 *          [영향] App.jsx 약 30곳 라벨, 실험실 그룹 8개, 카드 1개 제거 (-41라인)
 *                 로직 변경 없음, 라벨 일관성만 정리
 *          [백업] App.v2.7.0.backup.jsx
 * v2.7.0: [점수 시스템 전면 교체 — 돌파/강세 통일 (사용자 의도 + 실험실 검증)]
 *          [의도] 사용자: "돌파 강세 이렇게 이름 바꾸어서 전부 다 교체"
 *                 + 로직도 전부 교체 (실험실 데이터 기반 노이즈 제거)
 *          [명칭 변경 — 모든 탭]
 *                 🚀 돌파 (35+) → 🚀 돌파 (이벤트 + 점수)
 *                 💪 강세 (60+)  → 💪 강세 (지속 점수)
 *          [로직 변경 — getSignalsV2 재정의]
 *                 [돌파] = detectBreakoutEvent — 최근 3봉 내 구름 돌파 OR 스퀴즈 오프
 *                   • 노이즈 제거: ST flip(54%), MACD 골든(56%), ST 약세→3 점프
 *                   • 60% 승률 컴포넌트만 사용
 *                   • TRUE/FALSE 플래그 + 이벤트 정보 (며칠 전, 어떤 이벤트)
 *                 [강세] = calcSustainedStrength — 6 컴포넌트 합산 (만점 100)
 *                   • ST 3/3 (17점)
 *                   • MACD > Signal (17점)
 *                   • 가격 > MA20 (17점)
 *                   • 가격 > 3봉전 (17점)
 *                   • ST 카운트 ≥ 2 (16점)
 *                   • 거래량 ≥ 130% (16점)
 *                 [임계값] 강세 60+ = 통과, 80+ = 매우 강함
 *          [getSignalsV2 호환성]
 *                 sig.timing → sig.breakoutScore (이벤트 점수 환산: 100=오늘, 70=어제, 40=그제, 0=없음)
 *                 sig.strength → 새 calcSustainedStrength 점수
 *                 sig.timingOK → sig.breakoutOK (돌파 이벤트 + 강세 60+)
 *                 sig.strengthOK → sig.entryOK (그냥 강함, 강세 80+)
 *                 sig.ok → 둘 중 하나 충족
 *                 (기존 호출처는 그대로 작동, 라벨만 변경)
 *          [실험실 HIT 기준 변경]
 *                 옛: 🚀 돌파 + 💪 강세 60+
 *                 신규: 돌파 OR 강세 80+ (둘 중 하나)
 *          [영향] App.jsx 전체 (라벨 ~50곳, 로직 핵심 1곳, 다른 곳 자동 반영)
 *                 fetch_yahoo.py: 변경 없음 (텔레그램은 별도 작업)
 *          [백업] App.v2.6.6.backup.jsx, App.v2.6.7.backup.jsx
 ...
 *                 - 강세 단독 카드: 평균 +3.3% / 승률 63% (가장 좋음)
 *                 - 본인이 잘 매매한 GOOGL/DRAM/NVDA = 강세 단독 카테고리
 ...
 *          [집중탭 카드 2개 (기존 3 → 2)]
 *                 🚀 돌파 후 강함: detectBreakoutEvent + 강세 60+
 *                 💪 그냥 강함:    강세 80+ (이벤트 무관)
 *          [기존 시스템 호환]
 ...
 *                 - 🚀 돌파 패널 색상: 45/70 → 35/50 (35 진입 시 노랑 강조)
 *                 - 💪 강세 패널 색상: 50/70 → 60/75 (60 진입 시 초록 강조)
 ...
 *          [문제] v2.5.0의 60/60 통일 임계값에서 타이밍 평균이 ~18점에 머물러
 *                 종합평점 카드가 거의 항상 0개로 나옴 (강세장에서도)
 *          [원인 진단]
 *                 ① 거래량 컴포넌트가 게이트(volOK)와 중복 측정 — 노이즈 추가
 *                 ② 신선도 감점이 완만 (4일전도 40% 점수) — "오늘 발생"의 의미 약함
 *                 ③ 핵심 추세 전환 컴포넌트 (ST/MACD/구름) 점수가 작음
 *          [재설계 — calcEntryTiming]
 *                 ① ST 플립: 25→30pt (오늘 30 / 1일전 22 / 2일전 14 / 3-4일전 6)
 *                            ST 개선(2→3 아닌): 12→15 (15/10/5/0)
 *                 ② MACD 골든: 20→25pt (오늘 25 / 1일전 18 / 2일전 12 / 3-4일전 6)
 *                 ③ 구름 돌파: 15→20pt (오늘 20 / 1일전 13 / 2일전 8 / 3-4일전 4)
 *                 ④ 스퀴즈 해제: 15→유지 (오늘 15 / 1일전 10 / 2일전 5)
 *                 ⑤ RSI 50 돌파: 10→유지 (오늘 10 / 1일전 7 / 2일전 4 / 3-4일전 0)
 *                 ⑥ 거래량: 제거 (volOK 게이트와 중복)
 *                 합계 max = 100pt 정확히
 *          [임계값 비대칭] 타이밍 35 / 강도 60
 *                 - 타이밍 분포: 0~10(이벤트 없음 65%) / 15~25(약 25%) / 30~50(약 8%) / 50+(약 2%)
 *                 - 35 = "단일 강한 이벤트 + 약한 보조" 또는 "다중 약한 이벤트 발생"
 *                 - 60 도달은 다중 강한 이벤트 동시 발생 필요 — 너무 희소
 *                 - 강도 60은 그대로 (분포 자연스럽게 30~80점대 형성)
 *          [영향 영역] App.jsx 6곳
 *                 - 헤더 / calcEntryTiming / getSignalsV2 임계값 / 차트 매수 마커
 ...
 *                 텔레그램 알림 로직(fetch_yahoo.py)은 그대로 유지 — 화면만 통일
 *          [임계값] 타이밍 ≥ 60, 강도 ≥ 60 (이전 카드별 들쭉날쭉한 기준 통일)
 *                   거래량 지속 게이트 그대로 (3봉 중 2봉 100%↑) — 일시 폭증 필터링 유지
 *          [헬퍼] getSignalsV2(chartData) 추가 — 단일 진입점
 *                 반환: {ok, timing, strength, volSust, volOK, timingOK, strengthOK, ...}
 *                 ok = timingOK && strengthOK && volOK
 *          [집중탭 카드/펼침]
 *                 🟢 종합평점: getSignalsV2().ok (3조건 모두)
 *                 💎 돌파감지: timing ≥ 60 AND volOK
 *                 ⚡ 진입적기: strength ≥ 60 AND volOK
 *          [차트탭 종합판정] 45/50 → 60/60 통일
 *          [차트 매수 마커] ST flip + 신호 카운트 → 점수 임계값 (timing≥60 && strength≥60 && volOK)
 ...
 *          [수정] 펼침을 카드 정의와 일치시킴
 *                 - 돌파감지 펼침: 거래량 지속 AND 4중 2+ (ST/MACD/구름/스퀴즈)
 *                 - 진입적기 펼침: 거래량 지속 AND 4중 3+ (ST 3/3·MACD·MA20·모멘텀)
 *                 - 핵심매수(종합평점) 0개일 때 안내 메시지 추가
 *          [효과] 카드 카운트 = 펼친 리스트 개수 정확히 일치
 * v2.4.4: [집중탭 라벨 변경 — 핵심 매수 → 종합평점]
 *          [변경] 집중탭 카드 1: "🟢 핵심 매수" → "🟢 종합평점"
 *          [이유] 추천 표시 통일 일환 — 차트탭 종합판정과 의미 정렬
 ...
 *          [토글] 🚀 돌파 / 💪 강세 카드 — 클릭 시 컴포넌트별 점수 분해 표시
 ...
 *          [임계값] 130% (단봉) → 105% (3봉 중 2봉) — 약한 상승세로 조정
 *          [영향] 돌파/적기 시그널의 거래량 결합 조건 모두 변경
 *                 helper · 집중탭 카드 1/2/3 · 확장뷰 · 차트 매수 마커
 *                 종목 행 거래량 배지: "거래량 N%" → "거래량 N/3" (지속성)
 * v2.4.0: [시그널 시스템 재정의 — 수급 폐지 + 거래량 통합]
 *          [본질] 시그널 3개 → 2개로 단순화
 *                 수급 시그널 자체는 제거 (역할이 모호했음)
 *                 거래량 정보(120%↑)는 돌파/적기에 필수 조건으로 통합
 *          [핵심 2시그널]
 *                 💎 돌파: (ST flip / MACD골든 / 구름돌파 / 스퀴즈오프 중 2+) AND 거래량 120%↑
 *                 ⚡ 적기: (ST 3/3 + MACD + MA20 + 모멘텀 중 3+) AND 거래량 120%↑
 *          [데이터 근거] v2.3.14 Precision 측정 결과:
 *                       수급+적기 조합 = 손실 32~41% (가장 위험)
 *                       돌파+적기 조합 = 손실 1.5% (가장 안전)
 *                 → 수급 시그널의 차별성 약함, 거래량 정보만 가치 있음
 *          [영향 범위] App.jsx 9곳 + fetch_yahoo.py 3곳 변경
 *                     종목 행 배지: 핵심 3개 → 2개 (N/3 → N/2)
 *                     집중탭: "🟢 핵심매수" = 돌파+적기 (둘 다 충족)
 *                     실험실: 시그널 조합 7가지 → 4가지 (수급 빠진 조합)
 *                     차트: 매수 마커 5필터 → 4필터 (거래량 필수 + 나머지 4개)
 *                     알림: 슈퍼 등급 = 돌파+적기 (3 → 2 시그널 모두 충족)
 *          [호환성] sig_supply, supplyScore 변수 유지 (실험실 컴포넌트 카드 호환)
 ...
 *                   돌파: 4개 중 충족 개수
 *                   적기: 4개 중 충족 개수 (ST3/3+MACD+MA20위+모멘텀)
 *          [실험실] 신규/지속 구분 — 최근 3일 내 첫 발생 = 🆕 신규
 *          [실험실] 종목 행 배지: "수급3 🆕" / "돌파2" 형태로 강도 + 신규 동시 표시
 *          [실험실] 총 강도 N/12 표시 — 등급 판정 기준
 *          [목적] 같은 ✓라도 강도 다름 + 신규(타점) vs 지속(추격) 구분
 * v2.3.12: [시그널 순서 가설 검증 — 데이터로 가설 B 입증]
 *          [실험실] 시그널 이진(✓/✗) → 강도 점수 (각 0~4점, 총 0~12점)
 *                   수급: 거래량 단계(130/200/300%) + ST 3/3 보너스
 *                   돌파: 4개 중 충족 개수
 *                   적기: 4개 중 충족 개수 (ST3/3+MACD+MA20위+모멘텀)
 ...
 *          [목적] 같은 ✓라도 강도 다름 + 신규(타점) vs 지속(추격) 구분
 * v2.3.12: [시그널 순서 가설 검증 — 데이터로 가설 B 입증]
 *          [실험실] 각 핵심 시그널의 첫 발생 시점 별도 추적 — supplyDaysAgo / breakoutDaysAgo / entryDaysAgo
 *          [실험실] 시그널 순서 카드 추가 — 단계별 평균 d-X 표시
 *          [실험실] 가설 검증 자동 메시지 — 입증/부분/미입증 데이터로 판정
 *          [목적] 가설 입증 시 시스템 단계화 (🌱 수급 / 🚀 돌파 / 🎯 적기)
 * v2.3.11: [돌파감지 검증 완료 → 핵심 3시그널 시스템]
 *          [데이터] 돌파감지 HIT 58% vs MISS 0% (식별력 +58%p) — 핵심으로 승격
 *          [집중탭] 핵심 매수 = 수급+적기+돌파 중 2+ 충족 (3/3 충족 시 🚀 슈퍼 매수)
 *          [집중탭] 카드 부제 명확화 — 데이터 기반 핵심 시그널 명시
 *          [실험실] 종목 행에 💎 돌파 배지 추가 (놓친 부분 보정)
 *          [실험실] 핵심 시그널 카운트 N/5 → N/3 (수급+적기+돌파 핵심)
 *          [실험실] sigDefs 돌파감지 isCore:true 승격, "검증중" 라벨 제거
 *          [텔레그램] 🚀 슈퍼 등급 신설 — 핵심 3/3 + 보조 1+ 충족 시 별도 알림
 *          [Python] detect_buy_signal에 돌파감지 핵심 3번째 추가 (스퀴즈 제외)
...
function buildChartData(candles){
  ...
  // ★ v2.3.1: per-candle 구름 위치 + 거래량비 — 과거 시점 평가용 (이전엔 last만 있어서 발굴/리그에서 항상 false)
  const vols=candles.map(c=>c.volume||0);
  data.forEach((d,i)=>{
    const ct=d.spanA&&d.spanB?Math.max(d.spanA,d.spanB):null;
    d.aboveCloud=ct&&d.close>ct;
    d.nearCloud=ct&&!d.aboveCloud&&d.close>=ct*0.97;
    d.inCloud=ct&&d.close<=ct&&d.spanB&&d.close>=d.spanB;
    const ci3=i+off2;
    if(ci3>=20){
      const past=vols.slice(ci3-20,ci3).filter(v=>v>0);
      const avg=past.length?past.reduce((a,b)=>a+b,0)/past.length:0;
      d.volRatio=avg>0?+(vols[ci3]/avg*100).toFixed(0):100;
    }
  });
  // ★ v2.5.1: 매수 마커 — 점수 임계값 기반 통일 (옵션 B-1)
  //   진입 게이트: timing ≥ 35 AND strength ≥ 60 AND volOK (3봉 중 2봉 100%↑)
  //   등급:
  //     - 강력 (timing ≥ 60 AND strength ≥ 75): 다중 강한 이벤트 + 깊은 추세
  //     - 보통 (timing ≥ 35 AND strength ≥ 60): 단일 강한 이벤트 + 추세 형성
  //   data.slice(0, i+1)로 봉 시점별 점수 산정 (calcEntryTiming/Durability는 고정 lookback)
  for (let i = 10; i < data.length; i++) {
    const slice = data.slice(0, i + 1);
    const sig = getSignalsV2(slice);
    if (!sig.ok) continue;  // 게이트 미충족 = 마커 없음

    const c = data[i];
    const stFlip = i >= 1 && c.bullCount === 3 && data[i-1].bullCount < 3;
    const macdOK = c.macd != null && c.signal != null && c.hist != null && c.macd > c.signal && c.hist > 0;
    const rsiOK  = isRSIStrong(data, i);
    const cloudOK = !!c.aboveCloud || !!c.nearCloud;

    c.buySigCount  = [stFlip, macdOK, rsiOK, cloudOK].filter(Boolean).length;
    c.buySigDetail = {
      stFlip, macdOK, rsiOK, cloudOK,
      volOK: sig.volOK, volSust: sig.volSust,
      timing: sig.timing, strength: sig.strength,
      timingOK: sig.timingOK, strengthOK: sig.strengthOK,
    };

    if (sig.timing >= 60 && sig.strength >= 75) c.buyStrong = c.close;
    else c.buyNormal = c.close;
  }
  return data;
}

// ═══════════════════════════════════════════════════════════
// ★ v2.5.1: 진입타이밍 — "언제 시작했냐?" (시점 측정 전용)
// ═══════════════════════════════════════════════════════════
// 변경 (v2.5.0 → v2.5.1):
//   - 컴포넌트 재배점: ST 25→30, MACD 20→25, 구름 15→20 (시점 가중 강화)
//   - 거래량 컴포넌트 제거 (volOK 게이트와 중복 측정)
//   - 신선도 감점 가속 ("오늘 발생"에 큰 보상, 4일전은 낮게)
//   - 합계 max = 100pt 정확히 (5개 컴포넌트)
//
// 의미 분담:
//   - 타이밍 (이 함수): 언제 발생했나 (시점) — 이벤트 신선도만 측정
//   - 강도 (calcTrendDurability): 얼마나 견고한가 (지속성) — 상태값 누적
function calcEntryTiming(chartData) {
  ...
  // ① ST 플립 (최대 30pt) — v2.5.1: 25→30, 신선도 가속
  ...
  for (let ago = 0; ago < Math.min(5, L-1); ago++) {
    ...
    if (stNow === 3 && stPrev < 3) {
      ...
      stPts = ago === 0 ? 15 : ago === 1 ? 10 : ago === 2 ? 5 : 0;
      stLabel = `ST개선${ago===0?"":ago+"일전"}`;
      if (stPts > 0) { score += stPts; signals.push(stLabel); }
      break;
    }
  }
  breakdown.push({label: "ST 플립", desc: stLabel, pts: stPts, max: 30});

  // ② MACD 골든크로스 (최대 25pt) — v2.5.1: 20→25
  let macdPts = 0, macdLabel = "골든크로스 없음";
  for (let ago = 0; ago < Math.min(5, L-1); ago++) {
    const ci = L-1-ago, pi = ci-1;
    if (d[ci].macd > d[ci].signal && d[pi].macd <= d[pi].signal) {
      macdPts = ago === 0 ? 25 : ago === 1 ? 18 : ago === 2 ? 12 : 6;
      macdLabel = `MACD↑${ago===0?"오늘":ago+"일전"}`;
      score += macdPts; signals.push(macdLabel); break;
    }
  }
  breakdown.push({label: "MACD 골든", desc: macdLabel, pts: macdPts, max: 25});

  // ③ 구름 돌파 (최대 20pt) — v2.5.1: 15→20
  let cloudPts = 0, cloudLabel = "구름 돌파 없음";
  for (let ago = 0; ago < Math.min(5, L-1); ago++) {
    const ci = L-1-ago, pi = ci-1;
    if (d[ci].aboveCloud && !d[pi].aboveCloud) {
      cloudPts = ago === 0 ? 20 : ago === 1 ? 13 : ago === 2 ? 8 : 4;
      cloudLabel = `구름돌파${ago===0?"오늘":ago+"일전"}`;
      score += cloudPts; signals.push(cloudLabel); break;
    }
  }
  breakdown.push({label: "구름 돌파", desc: cloudLabel, pts: cloudPts, max: 20});

  // ④ 스퀴즈 해제 (최대 15pt) — v2.5.1: 신선도 감점 조정
  let sqzPts = 0, sqzLabel = "스퀴즈 미해제";
  for (let ago = 0; ago < Math.min(3, L); ago++) {
    if (d[L-1-ago].sqzOff) {
      sqzPts = ago === 0 ? 15 : ago === 1 ? 10 : 5;
  ...
  // ⑤ RSI 50 상향돌파 (최대 10pt) — v2.5.1: 신선도 감점 조정
  let rsiPts = 0, rsiLabel = `RSI ${d[L-1].rsi?.toFixed(0) || "—"}`;
  for (let ago = 0; ago < Math.min(5, L-1); ago++) {
    const ci = L-1-ago, pi = ci-1;
    if (d[ci].rsi >= 50 && d[pi].rsi < 50) {
      rsiPts = ago === 0 ? 10 : ago === 1 ? 7 : ago === 2 ? 4 : 0;
      rsiLabel = `RSI 50돌파${ago===0?"":ago+"일전"}`;
      if (rsiPts > 0) { score += rsiPts; signals.push(`RSI돌파50`); }
      break;
    }
  }
  ...
  // ⑥ 거래량 컴포넌트 제거 (v2.5.1)
  // 이유: getSignalsV2의 volOK 게이트(3봉 중 2봉 100%↑)와 중복 측정
  // 거래량은 게이트 역할만 하고, 시점 측정에서는 빠짐
  // breakdown.push({label: "거래량", desc: "게이트로 분리", pts: 0, max: 0}); // 기록 안 함

  const s = Math.min(100, Math.max(0, score));
  const grade = s >= 70 ? "🔥" : s >= 35 ? "⚡" : s >= 15 ? "💤" : "—";
...
// ═══════════════════════════════════════════════════════════
// ★ v2.5.0: getSignalsV2 — 화면 표시용 단일 진입점 (옵션 B-1)
// ═══════════════════════════════════════════════════════════
// 시그널 시스템(돌파N/적기N 카운트)을 점수 시스템(타이밍/강도)으로 통일
// 텔레그램 알림 로직은 fetch_yahoo.py에서 그대로 유지 — 화면만 통일
//
// 임계값: 타이밍 ≥ 60, 강도 ≥ 60
// 거래량 게이트: 최근 3봉 중 2봉 이상 volRatio ≥ 100% (지속성 — 일시 폭증 필터)
//
// 반환:
//   ok           — 종합평점 충족 (timingOK && strengthOK && volOK)
//   timing       — 0~100 (calcEntryTiming.score)
//   strength     — 0~100 (calcTrendDurability.score)
//   timingOK     — timing ≥ 60
//   strengthOK   — strength ≥ 60
//   volSust      — 0~3 (최근 3봉 중 100%↑ 봉 수)
//   volOK        — volSust ≥ 2
//   breakoutOK   — timingOK && volOK   (💎 돌파감지 카드용)
//   entryOK      — strengthOK && volOK (⚡ 진입적기 카드용)
//   timingSignals / strengthSignals    — 라벨 배열 (UI 보조 표시)
...
function getSignalsV2(chartData) {
  // ★ v2.7.0: 점수 시스템 전면 교체 — 돌파(이벤트) + 강세(점수)
  // 호환성을 위해 옛 필드명도 유지 (timing/strength 등) — 다른 곳 호출 보호
  const TH_BREAKOUT_STRONG = 60;  // 돌파 후 강함 = 강세 60+
  const TH_JUST_STRONG = 80;      // 그냥 강함 = 강세 80+ (이벤트 무관)
  const VOL_GATE = 100;
  const VOL_MIN_BARS = 2;

  if (!chartData || chartData.length < 10) {
    return {
      ok: false,
      // 새 시스템 필드
      breakout: false, breakoutScore: 0, breakoutEvents: [], breakoutDaysAgo: null,
      strength: 0, strengthBreakdown: [],
      // 호환 (옛 필드명) — 새 값 매핑
      timing: 0, timingOK: false, timingSignals: [], timingBreakdown: [],
      strengthOK: false,
      volSust: 0, volOK: false,
      breakoutOK: false, entryOK: false,
      threshold: { breakoutStrong: TH_BREAKOUT_STRONG, justStrong: TH_JUST_STRONG, volGate: VOL_GATE },
    };
  }

  // 1. 돌파 이벤트 발견
  const ev = detectBreakoutEvent(chartData);
  // 돌파 점수 환산 (UI 표시용): 오늘=100, 어제=70, 그제=40, 없음=0
  let breakoutScore = 0;
  if (ev.breakout && ev.daysAgo != null) {
    if (ev.daysAgo === 0) breakoutScore = 100;
    else if (ev.daysAgo === 1) breakoutScore = 70;
  ...
  for (let i = Math.max(0, chartData.length - 3); i < chartData.length; i++) {
    if ((chartData[i]?.volRatio || 0) >= VOL_GATE) volSust++;
  }
  const volOK = volSust >= VOL_MIN_BARS;

  // 4. 매수 신호 등급
  const breakoutOK = ev.breakout && strength >= TH_BREAKOUT_STRONG;  // 돌파 후 강함
  const entryOK = strength >= TH_JUST_STRONG;                        // 그냥 강함
  const ok = breakoutOK || entryOK;                                  // 둘 중 하나 충족

  // 5. 호환성 — 옛 코드가 timing/strengthOK 등 사용하는 곳 보호
  // timing 호환: 옛 ⚡ 점수 자리에 새 돌파 점수
  // strengthOK 호환: 옛 강세 60+ 자리에 새 강세 60+ (즉시 호환)
  const timingOK = ev.breakout;
  const strengthOK = strength >= TH_BREAKOUT_STRONG;

  // 신호 라벨 (UI용)
  const breakoutSignals = ev.events.map(e => e.label);
  const strengthSignals = ss.breakdown.filter(b => b.ok).map(b => b.label);

  return {
    ok,
    // 새 시스템 (메인)
    breakout: ev.breakout, breakoutScore, breakoutEvents: ev.events, breakoutDaysAgo: ev.daysAgo,
    strength, strengthBreakdown: ss.breakdown,
    // 호환 (옛 필드명) — 새 값으로 매핑
    timing: breakoutScore,
    timingOK, strengthOK,
    volSust, volOK,
    breakoutOK, entryOK,
    timingSignals: breakoutSignals,
    strengthSignals,
    timingBreakdown: ss.breakdown,  // 옛 timingBreakdown 자리에 새 강세 분해 (의미는 다르지만 표시 안전)
    threshold: { breakoutStrong: TH_BREAKOUT_STRONG, justStrong: TH_JUST_STRONG, volGate: VOL_GATE,
                 // 옛 임계값 호환
                 timing: TH_BREAKOUT_STRONG, strength: TH_BREAKOUT_STRONG },
  };
}
...
export default function App() {
  ...
  const [durabilityOpen, setDurabilityOpen] = useState(false); // ★ v2.4.3: 추세강세 펼치기
  ...
  useEffect(()=>{
    ...
    setPositions(prev=>prev.map(pos=>{
      ...
      // ★ v2.3.1: 추세추종 철학 — 참고가 도달은 청산 신호가 아님 (트레일링 스탑 단독)
      // 목표가 도달 알림 제거됨 — pos.target 필드는 시각화 참고용으로만 유지
      // ★ v2.3.1: 손절선 알림 — 이탈 확정/근접 분리 + 잔파동 음소거 (5% → 8% 리셋)
      const trailDist=pos.trailStop>0?((cur-newTrail)/cur*100):99;
      const broken=pos.trailStop>0&&cur<newTrail;
      if(broken&&!pos._stopBrokenAlerted){
        const breakPct=((cur-newTrail)/newTrail*100);
        const msg3a=`${pos.label} 🔴 손절선 이탈 확정! 현재 ${cur.toLocaleString()} < 손절 ${newTrail.toLocaleString()} (${breakPct.toFixed(1)}%)`;
        sendNotification("🔴 손절선 이탈", msg3a, `stop-broken-${pos.id}`);
        setAlerts(a=>[{id:Date.now(),type:"stop",msg:msg3a,ticker:pos.ticker,time:new Date().toLocaleTimeString("ko-KR")},...a].slice(0,20));
        pos._stopBrokenAlerted=true;
      }else if(trailDist>=5){pos._stopBrokenAlerted=false;}
      if(!broken&&trailDist<1&&trailDist>=0&&!pos._stopAlerted){
      ...
      }else if(trailDist>=8){pos._stopAlerted=false;}
      // ★ v2.2: 타임컷 판정 (박스권 감지)
...
개선점 3가지를 구체적으로 제안해줘.`;
  ...
  // ★ v2.3.1: ATR 기반 참고가 — 추세추종에 맞게 R:R 폐기
  // 매도 트리거는 트레일링 스탑 단독, 참고가는 시각적 진폭 참고용
  const lastAtr = charts[sel]?.data?.at(-1)?.atr || 0;
  const atrRef5 = lastAtr>0&&curPrice>0 ? +(curPrice + 5*lastAtr).toFixed(isKRSel?0:2) : 0;   // 단기 진폭 (1주 정도)
  const atrRef10= lastAtr>0&&curPrice>0 ? +(curPrice + 10*lastAtr).toFixed(isKRSel?0:2) : 0;  // 중기 진폭 (2주 정도)
  // ★ v2.3.1: 시각화 참고가 — 사용자설정 > 컨센서스 > 52주고점 > ATR×10 (R:R 후보 제거됨)
  const consTgtCalc = (()=>{
    ...
    const candidates = [
      selInfo?.target>0 && {price:selInfo.target, src:"설정참고"},
      w52High>curPrice*1.02 && {price:w52High, src:"52주고점"},
      atrRef10>0 && {price:atrRef10, src:"ATR×10"},
    ].filter(Boolean);
  ...
  const rrRatio  = 0; // v2.3.1: R:R 비율 폐기됨 — 추세추종은 트레일링 스탑 단독 청산
  ...
  if (!judgeGatePass) {
    ...
    if (selTiming.score >= 35 && selDurability.score >= 60) {
      ...
      judgeMsg = "강세 견고, 돌파 미흡 — 돌파 신호 대기";
    } else {
      judgeLabel = "⚪ 추세 미형성"; judgeColor = C.muted;
      judgeMsg = "강세 부족, 돌파만 발생 — 추세 형성 대기";
    }
  }
  ...
  const checkAutoVals = {
    ...
    sector: true,
    stock: selTiming.score >= 35 && selDurability.score >= 60,  // ★ v2.5.1: 35/60 비대칭 (시점/지속성)
      ...
      <div style={{flex:1,overflow:"auto"}} onClick={()=>setShowSearch(false)}>
        ...
        {tab==="radar"&&<div style={{padding:"10px 10px"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6,marginBottom:6}}>
            {[["^GSPC","S&P","🇺🇸"],["^IXIC","NDQ","🇺🇸"],["^KS11","KOSPI","🇰🇷"]].map(([k,name,flag])=>{
              ...
              return<div key={k} onClick={()=>setSelIndex(selIndex===k?null:k)} style={{border:`1px solid ${selIndex===k?C.accent:hasData?(pct>=0?"rgba(34,197,94,.3)":"rgba(255,69,58,.3)"):C.border}`,borderRadius:8,padding:"8px 10px",background:selIndex===k?"rgba(10,132,255,.08)":C.panel2,cursor:"pointer"}}>
                ...
                return<div key={mkt} style={{marginBottom:10}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:2}}>
                    {filtered.map((sec,i)=>{
                      ...
                      return<div key={sec.etf} onClick={()=>setSelectedSector(selectedSector===sec.etf?null:sec.etf)} style={{background:bg,borderRadius:4,padding:"4px 2px",border:selectedSector===sec.etf?`2px solid ${C.accent}`:isTop?`1px solid ${C.emerald}`:"1px solid rgba(255,255,255,.05)",cursor:"pointer",textAlign:"center"}}>
          ...
          <div style={css.card}>
            ...
            {(()=>{
              ...
              return breakouts.slice(0,8).map(s=>{
                ...
                return (
                <div key={s.ticker} onClick={()=>navigateToStock(s.ticker,s)} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",borderBottom:`1px solid rgba(255,255,255,.05)`,cursor:"pointer",background:isStrong?"rgba(48,209,88,.04)":"transparent"}}>
                  <span style={{fontWeight:900,fontSize:10,minWidth:60,maxWidth:82,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{fmtName(s)}</span>
                  <div style={{flex:1,display:"flex",gap:3,flexWrap:"wrap"}}>
                    {s.events.map((ev,i)=><span key={i} style={{fontSize:8,padding:"2px 6px",borderRadius:4,background:"rgba(255,159,10,.15)",border:`1px solid rgba(255,159,10,.4)`,color:"#FF9F0A",fontWeight:700}}>{ev.label}</span>)}
                    <span style={{fontSize:7,color:C.muted,padding:"2px 4px"}}>{dayLabel}</span>
                  </div>
                  <div style={{textAlign:"right"}}>
                    <div style={{fontSize:14,fontWeight:900,color:isStrong?C.emerald:C.muted}}>💪{s.strength}</div>
        ...
        {tab==="trade"&&<div style={{padding:"12px 14px"}}>
          ...
          <div style={{display:"flex",gap:4,marginBottom:10,flexWrap:"wrap"}}>
            {[["turnover","💰 거래대금"],["volRatio","📊 평소대비"],["timing","🚀 돌파"],["strength","💪 강세"],["chg","📈 1D"],["mktCap","🏢 시가총액"]].map(([k,l])=>(
          ...
          {(()=>{
            ...
            return <div style={{...css.card,padding:0,overflow:"hidden"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 12px",borderBottom:`1px solid ${C.border}`,background:"rgba(255,214,10,.04)"}}>
                ...
                <span style={{fontSize:8,color:C.muted}}>정렬: {{turnover:"💰 거래대금",volRatio:"📊 평소대비",timing:"🚀 돌파",strength:"💪 강세",chg:"📈 1D",mktCap:"🏢 시가총액"}[tradeSort]}</span>
              </div>
        ...
        {tab==="alpha"&&<div style={{padding:"12px 14px"}}>
          ...
          <div style={{display:"flex",gap:4,marginBottom:8}}>
            {[["all","전체"],["kr","🇰🇷 한국"],["us","🇺🇸 미국"]].map(([v,l])=>(
              <button key={v} onClick={()=>{setAlphaMarket(v);setFMarket(v);}} style={{padding:"4px 12px",borderRadius:5,border:`1px solid ${alphaMarket===v?C.accent:C.border}`,background:alphaMarket===v?"rgba(10,132,255,.12)":"transparent",color:alphaMarket===v?C.accent:C.muted,fontSize:9,fontWeight:alphaMarket===v?700:400,cursor:"pointer"}}>{l}</button>
            ))}
          </div>
          ...
          {alphaTab==="filter"&&<div>
            <div style={{background:C.panel,border:`1px solid ${C.border}`,borderRadius:10,padding:14,marginBottom:12}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
                ...
                <div>
                  <div style={{fontSize:9,fontWeight:600,color:C.sub,marginBottom:5}}>💪 RS강세 <span style={{color:C.accent}}>{fRS>0?`+${fRS}%p이상`:"전체"}</span></div>
            ...
            {alphaHits.length===0
              ...
              :<div>
                ...
                <div style={{display:"flex",gap:4,marginBottom:6,flexWrap:"wrap"}}>
                  {[["score","📊 종합"],["timing","🚀 돌파"],["entry","🎯 복합"],["accel","🔥 가속"],["rs","💪 RS"],["chg3d","📈 3D"],["vol","💥 거래량"]].map(([k,l])=>(
                ...
                <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10}}>
                  ...
                  <span style={{fontSize:9,fontWeight:700,color:C.accent,minWidth:28}}>{alphaMinScore}pt</span>
                  <span style={{fontSize:7,color:C.muted,marginLeft:"auto"}}>{alphaHits.filter(s=>s.score>=alphaMinScore).length}개 표시</span>
                </div>
                {[["🇰🇷 한국",true,"^KS11","KOSPI"],["🇺🇸 미국",false,"^GSPC","S&P 500"]].map(([label,isKR,idxKey,idxName])=>{
                  const hits=alphaHits.filter(s=>{const k=(s.market||"").includes("kr")||(s.ticker||"").length>5;return isKR?k:!k;}).filter(s=>s.score>=alphaMinScore);
                  ...
                  return<div key={label} style={{marginBottom:16}}>
                    ...
                    <div style={{overflowX:"auto"}}>
                      <table style={{width:"100%",borderCollapse:"collapse",fontSize:10,minWidth:480,tableLayout:"fixed"}}>
                        <thead>
                          <tr style={{background:"rgba(255,255,255,.04)",borderBottom:`2px solid ${C.border}`}}>
                            {["종목","점수","⚡","💪","1D","3D","5D","ST",""].map(h=>(
                              <th key={h} style={{padding:"5px 6px",textAlign:"left",color:C.purple,fontSize:8,fontWeight:700,whiteSpace:"nowrap"}}>{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          {hits.map((stock,i)=>{
                            ...
                            const tColor=stock.timing>=70?"#FF9F0A":stock.timing>=40?C.yellow:C.muted;
                            const dColor=stock.durability>=70?C.emerald:stock.durability>=50?C.green:C.muted;
                            return(
                              <tr key={stock.ticker} style={{background:stock.timing>=40&&stock.durability>=40?"rgba(48,209,88,.06)":stock.accelScore>=5?"rgba(255,159,10,.08)":isFlip?"rgba(48,209,88,.06)":i%2===0?C.panel:C.panel2,borderBottom:`1px solid ${C.border}`,cursor:"pointer"}}
                                ...
                                <td style={{padding:"6px 4px"}} onClick={e=>e.stopPropagation()}>
                                  <div style={{display:"flex",gap:2}}>
                                    ...
                                    <button onClick={()=>{inW?setWatchlist(w=>w.filter(x=>x.ticker!==stock.ticker)):setWatchlist(w=>[...w,{...stock}]);}} style={{background:inW?"rgba(10,132,255,.12)":"rgba(255,255,255,.04)",border:`1px solid ${inW?C.accent:C.border}`,color:inW?C.accent:C.muted,borderRadius:4,padding:"2px 5px",cursor:"pointer",fontSize:9}}>{inW?"★":"☆"}</button>
                                  </div>
                                </td>
                              </tr>
                            );
          ...
          {alphaTab==="compare"&&<div style={css.card}>
            <div style={{fontSize:11,fontWeight:700,color:C.accent,marginBottom:4}}>⚖️ 관심종목 RS강세 비교</div>
            {watchlist.length===0
              ...
              :<div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:10,minWidth:400}}>
                  <thead><tr style={{background:"rgba(255,255,255,.04)"}}>
                    {["종목","현재가","3D","5D","vs시장","🚀돌파","💪강세","관찰"].map(h=>(
                  ...
                  <tbody>
                    {[...watchlist].sort((a,b)=>((b.rs||0)-(a.rs||0))).map((stock,i)=>{
                      ...
                      const tm2=calcEntryTiming(cdc);
                      const dr2=calcTrendDurability(cdc);
                      return(
        ...
        {tab==="sniper"&&selInfo&&<div style={{padding:"12px 14px"}}>
          ...
          {/* ★ v2.3.1: 시뮬 데이터 강한 경고 — 가짜 캔들 기반 분석은 의미 없음 */}
          {cd&&!cd.real&&<div style={{background:"rgba(255,214,10,.1)",border:`1px solid ${C.yellow}`,borderRadius:6,padding:"8px 12px",marginBottom:10,display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:14}}>⚠️</span>
            <div style={{flex:1}}>
              <div style={{fontSize:9,fontWeight:700,color:C.yellow}}>시뮬레이션 차트 — 랜덤 워크로 생성된 가짜 캔들</div>
              <div style={{fontSize:7,color:C.muted,marginTop:2}}>아래 ST/MACD/RSI 등 모든 지표는 가짜 데이터 기반. <b>실제 매매 판단 금지</b>. 위 "🔄 실시간 전환" 버튼으로 실제 차트 받으세요.</div>
            </div>
          </div>}

          {/* ★ v2.5.2: ST 추세 건강 인디케이터 제거 — 정보가 💪강세 컴포넌트(ADX/EMA/ST유지)와 중복 */}

          {/* ★ v2.5.2: 매수 신호 5필터 박스 제거 — ST/MACD/RSI/거래/구름 정보가 🚀돌파/💪강세 컴포넌트와 중복 */}
          ...
          {companyInfo[sel]&&(()=>{
            ...
            return<div style={{...css.card,marginBottom:10,border:`1px solid rgba(10,132,255,.2)`}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
              ...
              <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                {naverUrl&&<a href={naverUrl} target="_blank" rel="noopener noreferrer" style={{fontSize:8,padding:"4px 10px",borderRadius:5,background:"rgba(3,199,90,.1)",border:"1px solid rgba(3,199,90,.3)",color:"#03C75A",fontWeight:700,textDecoration:"none"}}>📊 네이버증권</a>}
          ...
          {/* ★ v2.4.3: 🚀 돌파 + 💪 강세 통합 패널 — 클릭 시 컴포넌트별 펼치기 */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:10}}>
            ...
            <div style={{background:selTiming.score>=35?"rgba(255,159,10,.08)":"rgba(255,255,255,.03)",border:`2px solid ${selTiming.score>=50?"#FF9F0A":selTiming.score>=35?C.yellow:"rgba(255,255,255,.1)"}`,borderRadius:10,padding:"10px 12px",cursor:"pointer"}} onClick={()=>setTimingOpen(!timingOpen)}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                <span style={{fontSize:9,fontWeight:700,color:"#FF9F0A"}}>🚀 돌파 {timingOpen?"▼":"▶"}</span>
            ...
            <div style={{background:selDurability.score>=60?"rgba(48,209,88,.06)":"rgba(255,255,255,.03)",border:`2px solid ${selDurability.score>=75?C.emerald:selDurability.score>=60?C.green:"rgba(255,255,255,.1)"}`,borderRadius:10,padding:"10px 12px",cursor:"pointer"}} onClick={()=>setDurabilityOpen(!durabilityOpen)}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                <span style={{fontSize:9,fontWeight:700,color:C.emerald}}>💪 강세 {durabilityOpen?"▼":"▶"}</span>
            ...
            return<div style={{...css.card,marginBottom:10,padding:"10px 12px"}}>
              ...
              {priceLevelOpen && <div style={{position:"relative"}}>
                {levels.map((lv,i)=>{
                  ...
                  return<div key={i} style={{display:"flex",alignItems:"center",gap:6,padding:"3px 0",borderBottom:`1px solid rgba(255,255,255,.04)`,background:isCurrent?"rgba(10,132,255,.08)":"transparent"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:6,padding:"4px 0",background:"rgba(10,132,255,.12)",borderRadius:4,marginTop:4}}>
          ...
          {/* ★ v2.3.1: 컨센서스 참고가 + 진척률 (R:R 폐기, 추세추종 철학) */}
          <div style={{background:"linear-gradient(135deg,rgba(10,132,255,.08),rgba(191,90,242,.08))",border:`2px solid ${consTgtSrc==="컨센서스"?C.accent:consTgt>0?"rgba(255,214,10,.4)":"rgba(255,255,255,.12)"}`,borderRadius:12,padding:"14px 16px",marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
              <div>
                <div style={{fontSize:9,color:C.muted,marginBottom:4}}>📍 참고가 {consTgtSrc&&<span style={{color:C.accent,fontSize:7}}>({consTgtSrc})</span>}
                  <button onClick={()=>{const v=prompt(`참고가 입력 (${isKRSel?"원":"USD"}):`,consTgt>0?Math.round(consTgt):"");if(v&&+v>0)setUserTargets(p=>({...p,[sel]:+v}));}} style={{marginLeft:4,background:"none",border:`1px solid ${C.border}`,borderRadius:3,padding:"1px 5px",color:C.accent,fontSize:7,cursor:"pointer"}}>✏️ 수정</button>
              ...
              <div style={{textAlign:"right"}}>
                <div style={{fontSize:9,color:C.muted,marginBottom:4}}>참고가 대비</div>
                {consTgt>0&&curPrice>0?<>
                  ...
                  <div style={{fontSize:7,color:C.muted,marginTop:4}}>매도 트리거 ≠ 참고가<br/>트레일링 스탑 단독</div>
            ...
            {/* ATR 도달 예상 */}
            {atrDaysToTarget&&<div style={{fontSize:8,color:C.muted,marginTop:6,textAlign:"center"}}>📈 ATR 기준 참고가 도달 예상: <span style={{color:C.accent,fontWeight:700}}>약 {atrDaysToTarget}거래일</span> (일변동 {atrDaily}%)</div>}
          </div>

          {/* ★ v2.3.1: 참고가 3종 — ATR 기반 진폭 + 52주 고점 (R:R 폐기) */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:8}}>
            <div style={{background:"rgba(10,132,255,.08)",border:`1px solid rgba(56,189,248,.3)`,borderRadius:8,padding:"9px 10px",textAlign:"center"}}>
              <div style={{fontSize:8,color:C.muted,marginBottom:2}}>단기 진폭 (5×ATR)</div>
              <div style={{fontSize:14,fontWeight:800,color:C.accent}}>{atrRef5>0?`${unit}${fmtPrice(atrRef5,isKRSel)}`:"—"}</div>
              {atrRef5>0&&curPrice>0&&<div style={{fontSize:9,color:C.green,marginTop:2}}>+{((atrRef5-curPrice)/curPrice*100).toFixed(1)}%</div>}
            </div>
            <div style={{background:"rgba(191,90,242,.08)",border:"1px solid rgba(191,90,242,.3)",borderRadius:8,padding:"9px 10px",textAlign:"center"}}>
              <div style={{fontSize:8,color:C.muted,marginBottom:2}}>중기 진폭 (10×ATR)</div>
              <div style={{fontSize:14,fontWeight:800,color:C.purple}}>{atrRef10>0?`${unit}${fmtPrice(atrRef10,isKRSel)}`:"—"}</div>
              {atrRef10>0&&curPrice>0&&<div style={{fontSize:9,color:C.green,marginTop:2}}>+{((atrRef10-curPrice)/curPrice*100).toFixed(1)}%</div>}
            </div>
          ...
          <div style={{fontSize:7,color:C.muted,marginBottom:8,textAlign:"center",lineHeight:1.4}}>
            📌 위 가격은 <b>참고용 진폭</b>이지 청산 신호 아님 · <b>매도는 트레일링 스탑</b>이 단독으로 결정 (추세 살아있는 한 보유)
          </div>
          ...
          {cd?.data?.length>10&&(()=>{
            ...
            return<div style={{...css.card,marginBottom:10}}>
              ...
              <div style={{position:"relative",height:28,background:"rgba(255,255,255,.04)",borderRadius:6,marginBottom:4,overflow:"hidden"}}>
                <div style={{position:"absolute",left:0,top:0,height:"100%",width:`${Math.min(100,pct)}%`,background:"linear-gradient(90deg,rgba(255,69,58,.2),rgba(48,209,88,.2))",borderRadius:6}}/>
              ...
              {atrDaily&&<div style={{display:"flex",gap:8,fontSize:8,color:C.muted}}>
                ...
                {atrDaysToTarget>0&&<span>참고가까지 <span style={{color:C.emerald,fontWeight:700}}>~{atrDaysToTarget}일</span></span>}
              </div>}
            </div>;
          })()}
          ...
          {!tracking.find(t=>t.ticker===sel)&&!positions.find(p=>p.ticker===sel)&&<button onClick={()=>{
            ...
            setTracking(p=>[...p,{id:Date.now(),ticker:sel,label:selInfo.label,market:selInfo.market,basePrice:curPrice,addedDate:new Date().toLocaleDateString("ko-KR"),foundScore:score,foundSignals:signals,foundRS:rs,foundTiming:tm2.score,foundDurability:dr2.score,oppScoreAt:oppScore}]);
            setAddMsg(`👁 ${selInfo.label} 관찰 등록 (⚡${tm2.score} 💪${dr2.score})`);setTimeout(()=>setAddMsg(""),3000);
          }} style={{width:"100%",background:"rgba(10,132,255,.08)",border:`1px solid ${C.accent}`,borderRadius:8,padding:"10px",color:C.accent,fontWeight:700,fontSize:11,cursor:"pointer",marginBottom:10}}>
            👁 관찰 등록 — 추적탭에서 모니터링
          </button>}
          {tracking.find(t=>t.ticker===sel)&&<div style={{textAlign:"center",padding:"8px",marginBottom:10,borderRadius:8,background:"rgba(10,132,255,.06)",border:`1px solid ${C.accent}`}}>
          ...
          <div style={{...css.card,marginBottom:10,border:`1px solid ${checkOk?C.emerald:C.border}`}}>
            ...
            {[
              ...
              ["stock",selTiming.score>=35&&selDurability.score>=60,"📈 돌파 "+selTiming.score+" · 강세 "+selDurability.score+" (🚀 돌파 + 💪 강세 60+)"],
                  ...
                  return<div key={t.id||i} style={{...css.card,border:`1px solid ${chg>=5?C.emerald:chg<=-3?C.red:C.border}`}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr 1fr",gap:4,marginBottom:8}}>
                      <div style={{background:"rgba(255,159,10,.06)",borderRadius:5,padding:"4px",textAlign:"center",border:`1px solid ${tm.score>=35?"#FF9F0A":"transparent"}`}}>
                        <div style={{fontSize:6,color:C.muted}}>🚀돌파</div>
                      ...
                      <div style={{background:"rgba(48,209,88,.06)",borderRadius:5,padding:"4px",textAlign:"center",border:`1px solid ${dr.score>=60?C.emerald:"transparent"}`}}>
                        <div style={{fontSize:6,color:C.muted}}>💪강세</div>
                    ...
                    {tm.score>=35&&dr.score>=60&&<div style={{background:"rgba(48,209,88,.08)",borderRadius:5,padding:"4px 8px",marginBottom:6,textAlign:"center"}}>
                      <span style={{fontSize:8,fontWeight:700,color:C.emerald}}>✅ 진입 조건 충족 (🚀 돌파 + 💪 강세 60+) — 매수 전환 검토</span>
                    </div>}
                    <div style={{display:"flex",gap:6}}>
                      ...
                      <button onClick={()=>{
                        ...
                        setTracking(p=>p.filter((_,j)=>j!==i));
                      }} style={{background:"rgba(255,255,255,.05)",border:`1px solid ${C.border}`,color:C.muted,borderRadius:6,padding:"6px 8px",cursor:"pointer",fontSize:9}}>✕</button>
                    </div>
                  </div>;
                })}
              </div>
                ...
                {positions.map(pos=>{
                  ...
                  return<div key={pos.id} style={{...css.card,border:`2px solid ${near?"rgba(255,69,58,.8)":isTimeCut?"rgba(255,159,10,.7)":volDrop?"rgba(250,204,21,.6)":pos.trailMode?"rgba(250,204,21,.5)":C.border}`,animation:near?"ap 2s infinite":""}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:10}}>
                      ...
                      {/* 참고가 진척률 */}
                      <div style={{background:"rgba(0,0,0,.5)",borderRadius:8,padding:"8px",textAlign:"center"}}>
                        <div style={{fontSize:7,color:C.muted,fontWeight:700}}>📍 참고가</div>
        ...
        {tab==="lab"&&<div style={{padding:"12px 14px"}}>
          ...
          <div style={{fontSize:9,color:C.sub,marginBottom:8}}>v2.5.9: 최근 오른 종목들의 <b>최적 진입 시점</b> 자동 탐색 — d-30 ~ d-2 스캔, 🚀 돌파 60+ AND 💪강세 60+ 충족된 첫 봉 (앞 탭들과 동일 기준)</div>
          <div style={{display:"flex",gap:4,marginBottom:10}}>
            {[["all","전체"],["kr","🇰🇷 한국"],["us","🇺🇸 미국"]].map(([v,l])=>(
          ...
          {(()=>{
            ...
            const analyzed = Object.entries(charts)
              ...
              .map(([ticker,c])=>{
                ...
                const helper = (idx) => {
                  ...
                  else if (volR >= 300) supplyScore += 3;
                  if (stC === 3) supplyScore += 1;
                  // 돌파 강도: 4개 중 충족 개수
                  ...
                  if (prev) {
                    ...
                    if (candle.sqzOff && prev.sqzOn) breakoutCount++;
                  }
                  const breakoutScore = breakoutCount;
                  // ★ v2.4.0: 돌파 = 4개 중 2+ AND 거래량 120%↑ 필수
                  const sig_breakout = breakoutCount >= 2 && volOK;
                  // 적기 강도: 4개 중 충족 개수
                  const ma20Now = candle.ma20;
                  const aboveMa20 = ma20Now != null && candle.close > ma20Now;
                  const macdOK = candle.macd != null && candle.signal != null && candle.macd > candle.signal;
                  const momentumOK = idx >= 3 ? candle.close > d[idx-3].close : false;
                  const entryComponents = [stC === 3, macdOK, aboveMa20, momentumOK];
                  const entryScore = entryComponents.filter(Boolean).length;
                  // ★ v2.4.0: 적기 = 4개 중 3+ AND 거래량 120%↑ 필수
                  const sig_entry = entryScore >= 3 && volOK;

                  // ★ v2.7.0: 점수 시스템 전면 교체 — 돌파(이벤트) + 강세(점수)
                  // sig_timing = 돌파 이벤트 (이름은 호환 유지, 의미만 변경)
                  // sig_strength = 새 강세 점수 60+
                  // 옛 calcEntryTiming/calcTrendDurability는 timingPt/durPt 변수에 그대로 보존 (LEGACY 분석용)
                  let sig_timing_new = false;
                  if (idx >= 1 && idx < d.length) {
                    const dCutForEv = d.slice(0, idx + 1);
                    const ev = detectBreakoutEvent(dCutForEv);
                    sig_timing_new = ev.breakout;
                  }
                  const ssScore = (() => {
                    if (idx < 1) return 0;
                    const dCutForSS = d.slice(0, idx + 1);
                    return calcSustainedStrength(dCutForSS).score || 0;
                  })();
                  const sig_timing   = sig_timing_new;        // 🚀 돌파 이벤트
                  const sig_strength = ssScore >= 60;         // 💪 강세 60+
                ...
                return {
                  ...
                  daysAgo, // 신호 발생 며칠 전
                  // ★ v2.3.12: 각 시그널 첫 발생 시점
                  supplyDaysAgo: firstSupplyIdx !== null ? L - 1 - firstSupplyIdx : null,
                  breakoutDaysAgo: firstBreakoutIdx !== null ? L - 1 - firstBreakoutIdx : null,
                  entryDaysAgo: firstEntryIdx !== null ? L - 1 - firstEntryIdx : null,
                  // ★ v2.3.13 (B 강도 + C 신규/지속)
                  supplyScore: sigDay.supplyScore,    // 0~4
                  breakoutScore: sigDay.breakoutScore, // 0~4
                  entryScore: sigDay.entryScore,       // 0~4
                  totalIntensity: sigDay.totalIntensity, // 0~12
                  volR: sigDay.volR,
                  volSustainedCount: sigDay.volSustainedCount, // v2.4.1: 거래량 지속 봉 수
                  // 신규/지속 — 최근 3일 내 첫 발생 = 신규
            ...
            const combos = [
              {name:"🌟 둘 다 충족 (🚀 돌파 + 💪 강세 60+)", filter:(b)=>b.sig_timing&&b.sig_strength&&b.sig_volume, color:"#FFD60A"},
              {name:"🚀 돌파 단독 (이벤트 but 강세 부족)",     filter:(b)=>b.sig_timing&&!b.sig_strength,            color:"#FF9F0A"},
              {name:"💪 강세 단독 (이벤트 X but 강세 충분)",     filter:(b)=>b.sig_strength&&!b.sig_timing,            color:C.emerald},
            ...
            const components = [
              ...
              {key:"c_st3",        label:"ST 3/3",         group:"강세", color:"#FF9F0A"},
              {key:"c_macdPos",    label:"MACD > Signal",  group:"강세", color:"#FF9F0A"},
              {key:"c_aboveMa20",  label:"가격 > MA20",     group:"강세", color:"#FF9F0A"},
              {key:"c_momentum",   label:"가격 > 3봉전",    group:"강세", color:"#FF9F0A"},
            ...
            const sigDefs = [
              {key:"sig_timing",   label:"🚀 돌파 35+",   isCore:true},
              {key:"sig_strength", label:"💪 강세 60+",     isCore:true},
            ...
            // 잘 잡은 패턴 — HIT 종목에서 가장 자주 충족된 시그널
            const hitAnalysis = hits.length>0 ? sigDefs.map(s=>({
            ...
            // ★ v2.3.12: 시그널별 평균 발생 시점 (가설 검증)
            const supplyHits = hits.filter(h=>h.supplyDaysAgo!=null);
            const breakoutHits = hits.filter(h=>h.breakoutDaysAgo!=null);
            const entryHits = hits.filter(h=>h.entryDaysAgo!=null);
            const avgSupplyDays = supplyHits.length>0 ? +(supplyHits.reduce((s,x)=>s+x.supplyDaysAgo,0)/supplyHits.length).toFixed(1) : null;
            const avgBreakoutDays = breakoutHits.length>0 ? +(breakoutHits.reduce((s,x)=>s+x.breakoutDaysAgo,0)/breakoutHits.length).toFixed(1) : null;
            const avgEntryDays = entryHits.length>0 ? +(entryHits.reduce((s,x)=>s+x.entryDaysAgo,0)/entryHits.length).toFixed(1) : null;
            ...
            return <>
              ...
              <div style={{...css.card, border:`2px solid ${hitRate>=60?C.emerald:hitRate>=40?C.yellow:C.red}`, background:"linear-gradient(135deg,rgba(191,90,242,.04),rgba(48,209,88,.04))"}}>
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,marginBottom:10}}>
                  ...
                  <div style={{textAlign:"right"}}>
                    ...
                    <div style={{fontSize:7,color:C.muted,marginTop:4}}>HIT = 🚀 돌파 + 💪 강세 60+</div>
                  </div>
                </div>
                ...
                {/* ★ v2.3.12: 시그널 순서 검증 — 가설: 수급 → 돌파 → 적기 순서 */}
                {hits.length>0&&(avgSupplyDays!==null||avgBreakoutDays!==null||avgEntryDays!==null)&&<div style={{borderTop:`1px solid ${C.border}`,marginTop:8,paddingTop:8}}>
                  <div style={{fontSize:8,color:C.muted,textAlign:"center",marginBottom:6,fontWeight:700}}>🔬 시그널 발생 순서 — 가설: 수급(전조) → 돌파(시작) → 적기(확정)</div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
                    <div style={{textAlign:"center",background:"rgba(48,209,88,.06)",border:"1px solid rgba(48,209,88,.3)",borderRadius:6,padding:"6px"}}>
                      <div style={{fontSize:7,color:C.emerald,fontWeight:700}}>🟢 수급 (1단계)</div>
                      <div style={{fontSize:14,fontWeight:900,color:C.emerald}}>{avgSupplyDays!==null?`d-${avgSupplyDays.toFixed(1)}`:"—"}</div>
                      <div style={{fontSize:6,color:C.muted}}>{supplyHits.length}/{hits.length}개 충족</div>
                    </div>
                    <div style={{textAlign:"center",background:"rgba(100,210,255,.06)",border:"1px solid rgba(100,210,255,.3)",borderRadius:6,padding:"6px"}}>
                      <div style={{fontSize:7,color:"#64D2FF",fontWeight:700}}>💎 돌파 (2단계)</div>
                      <div style={{fontSize:14,fontWeight:900,color:"#64D2FF"}}>{avgBreakoutDays!==null?`d-${avgBreakoutDays.toFixed(1)}`:"—"}</div>
                      <div style={{fontSize:6,color:C.muted}}>{breakoutHits.length}/{hits.length}개 충족</div>
                    </div>
                    <div style={{textAlign:"center",background:"rgba(255,159,10,.06)",border:"1px solid rgba(255,159,10,.3)",borderRadius:6,padding:"6px"}}>
                      <div style={{fontSize:7,color:"#FF9F0A",fontWeight:700}}>⚡ 적기 (3단계)</div>
                      <div style={{fontSize:14,fontWeight:900,color:"#FF9F0A"}}>{avgEntryDays!==null?`d-${avgEntryDays.toFixed(1)}`:"—"}</div>
                      <div style={{fontSize:6,color:C.muted}}>{entryHits.length}/{hits.length}개 충족</div>
                    </div>
                  </div>
                  {/* 가설 검증 결과 */}
                  {avgSupplyDays!==null&&avgBreakoutDays!==null&&avgEntryDays!==null&&(()=>{
                    const ordered = avgSupplyDays > avgBreakoutDays && avgBreakoutDays > avgEntryDays;
                    const partOrdered = avgSupplyDays > avgEntryDays;
                    return <div style={{fontSize:7,marginTop:6,padding:"4px 8px",background:ordered?"rgba(48,209,88,.08)":partOrdered?"rgba(255,159,10,.08)":"rgba(255,69,58,.08)",borderRadius:4,textAlign:"center",color:ordered?C.emerald:partOrdered?"#FF9F0A":C.red,fontWeight:700}}>
                      {ordered ? `✅ 가설 입증 — 수급 → 돌파 → 적기 순서대로 발생 (간격: 수급→돌파 ${(avgSupplyDays-avgBreakoutDays).toFixed(1)}일, 돌파→적기 ${(avgBreakoutDays-avgEntryDays).toFixed(1)}일)`
                       : partOrdered ? `🟡 부분 입증 — 수급이 적기보다 ${(avgSupplyDays-avgEntryDays).toFixed(1)}일 빠름 (돌파는 비순서)`
                       : `❌ 가설 미입증 — 순서 불명확 (시그널들이 거의 동시 발생)`}
                    </div>;
                  })()}
                </div>}
              </div>
              ...
              {misses.length>0 && <div style={css.card}>
                <div style={{fontSize:10,fontWeight:700,color:C.red,marginBottom:4}}>❌ 놓친 이유 — MISS {misses.length}개 시작 시점 시그널 충족률</div>
              ...
              {/* TOP 종목 목록 — HIT/MISS 표시 + 점수 시스템 통일 (v2.5.1) */}
              <div style={css.card}>
                <div style={{fontSize:10,fontWeight:700,color:"#FF9F0A",marginBottom:4}}>🏆 최근 5일 상승 TOP {totalTop} — 시그널 발생 시점 자동 탐색</div>
                <div style={{fontSize:7,color:C.muted,marginBottom:8}}>v2.5.1: 🚀 돌파 (이벤트) · 💪강세 60+ (지속 점수) · 거래량 지속 (3봉 중 2봉↑ 100%) 게이트 · "d-X" = 며칠 전 신호 발생</div>
                <div style={{maxHeight:400,overflowY:"auto"}}>
                  {top40.map((s,i)=>{
                    const isKR4=(s.ticker?.length||0)>5;
                    // ★ v2.5.1: 점수 시스템 — 돌파 (이벤트) / 강세 60+ (지속 점수) 비대칭
                    const tPt = s.timingPt || 0;
                    const dPt = s.durPt || 0;
                    const tOK = tPt >= 35;
                    const dOK = dPt >= 60;
                    const volOK = (s.volSustainedCount || 0) >= 2;
                    const okCount = [tOK, dOK, volOK].filter(Boolean).length;
                    return <div key={s.ticker} onClick={()=>navigateToStock(s.ticker,{ticker:s.ticker,label:s.label,market:s.market})} style={{display:"flex",alignItems:"center",gap:5,padding:"6px",borderBottom:`1px solid rgba(255,255,255,.04)`,cursor:"pointer",background:s.isHit?"rgba(48,209,88,.04)":"rgba(255,69,58,.02)"}}>
                      ...
                      {/* ★ v2.5.1: 점수 배지 — 🚀돌파 (35+ 강조) / 💪강세 (60+ 강조) */}
                      <div style={{display:"flex",gap:2,flexShrink:0}}>
                        <span title={`🚀 돌파 ${tPt}/100 (≥35 시점 충족)`} style={{fontSize:7,padding:"2px 5px",borderRadius:3,background:tOK?"rgba(255,159,10,.25)":"rgba(255,255,255,.04)",color:tOK?"#FF9F0A":C.muted,fontWeight:900,border:`1px solid ${tOK?"#FF9F0A":C.border}`}}>⚡{tPt}</span>
                        <span title={`💪 강세 ${dPt}/100 (≥60 지속성 충족)`} style={{fontSize:7,padding:"2px 5px",borderRadius:3,background:dOK?"rgba(48,209,88,.25)":"rgba(255,255,255,.04)",color:dOK?C.emerald:C.muted,fontWeight:900,border:`1px solid ${dOK?C.emerald:C.border}`}}>💪{dPt}</span>
                      </div>
                      <div style={{display:"flex",gap:1,flex:1}}>
                        <span title={`거래량 지속성 ${s.volSustainedCount}/3봉 (100%↑) — 현재 ${s.volR}% · 2/3 이상 = 게이트 통과`} style={{fontSize:6,padding:"1px 3px",borderRadius:2,background:volOK?"rgba(48,209,88,.15)":"rgba(255,255,255,.04)",color:volOK?C.emerald:C.muted,fontWeight:700}}>📊거래량 {s.volSustainedCount}/3</span>
                      </div>
                      <span title="3 게이트 충족 (🚀 돌파 · 💪 강세 60+ 또는 강세 80+)" style={{fontSize:8,fontWeight:700,color:okCount===3?C.emerald:okCount===2?"#FF9F0A":okCount===1?C.yellow:C.muted,minWidth:25,textAlign:"center"}}>{okCount}/3</span>
              ...
              {/* 처방 카드 */}
              <div style={{...css.card, background:"linear-gradient(135deg,rgba(191,90,242,.06),rgba(10,132,255,.06))",border:`1px solid ${C.purple}`}}>
                <div style={{fontSize:10,fontWeight:700,color:C.purple,marginBottom:6}}>💊 처방</div>
                <div style={{fontSize:8,color:C.sub,lineHeight:1.6}}>
                  적중률 <b style={{color:hitRate>=60?C.emerald:hitRate>=40?C.yellow:C.red}}>{hitRate}%</b>
                  {hitRate>=60 ? <> · 현재 시그널 조합이 잘 작동 중. 임계값 유지.</>
