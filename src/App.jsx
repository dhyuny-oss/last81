 ...
 *                 신규 컴포넌트 단독 효과 측정 결과:
 *                   - ⭐ ADX 25+ : +2.3% / 58% (예상보다 약함)
 *                   - ⭐ MA20+기울기↑: +3.2% / 61% (예상대로)
 *                   - ⭐ 52주 -15% : +2.6% / 62% (안전)
 *                 → ADX 가중치 과대평가 확인, 재조정 필요
 *          [Q1=A: 가중치 재조정]
 *                 큰 변화:
 *                   ADX 25+              18 → 6  (-12) ⭐ 효과 약함 입증
 *                   ST 3/3 유지          10 → 15 (+5) ⭐ 효과 +3.6% 최고
 *                   거래량 130% 재추가   —  → 9  (+9) ⭐ 효과 +3.3%, 63%
 *                 미세 조정:
 *                   가격>MA20+기울기↑    14 → 13
 *                   ST 3/3               12 → 11
 *                   52주 -15%            14 → 10
 ...
 *                   RSI 50~70             6 → 4
 *          [Q2=C: 신규 컴포넌트 2개 추가]
 *                 - MA5 > MA20 (6점): 단기 모멘텀 (며칠~1개월 매매 정합)
 *                 - +DI > -DI  (5점): ADX 보조 (방향성 확인)
 *          [Q3=E: 추가 지표 검토는 1주 후로]
 *          [Q4=A: 1주 검증 후 재분석]
 *          [실험실 컴포넌트 17 → 19]
 *                 - c_ma5gt20 (MA5 > MA20) 추가
 *                 - c_pdiPos (+DI > -DI) 추가
 *          [영향]
 *                 - calcSustainedStrength: 9 → 12 컴포넌트
 *                 - 가중치 분포 더 평탄해짐 (최대 15 → 4)
 *                 - 둘 다 충족 조합 효과 측정 표본 확대 예상
 *          [백업] App.v2.7.4.backup.jsx
 *          [한계 짚기]
 *                 - 표본 크기 ~3000개 (통계적 유의 가능 단 시장 의존)
 *                 - 신규 2개는 검증 안 됨 (1주 후 결과 봐야)
 *                 - 5일 후 측정 → 7일/10일도 봐야 정확
 ...
 *                 못 매매: KO/WMB/VLO/WELL — 횡보주, 52주 멀음, ADX 약함
 *                 → 추세추종 핵심 지표(ADX/52주/MA20기울기) 가중치 강화
 *          [B 안: 새 지표 발굴]
 *                 추가:
 *                   ⑴ ADX 25+              (20점) — 추세 강도 핵심
 *                   ⑵ 가격>MA20 + 기울기↑   (15점) — 진짜 상승 추세
 *                   ⑶ 52주 고점 -15% 이내   (15점) — 강세 지속성
 *                   ⑹ 거래량 5일 > 20일     (10점) — 거래량 품질
 *                   ⑺ OBV 동행 (5일)        (8점) — 거래량 누적
 *                   ⑻ RSI 50~70             (8점) — 모멘텀 영역
 *                 제거:
 *                   ❌ 가격>3봉전 (가격>MA20과 중복)
 *                   ❌ ST 카운트≥2 (ST 3/3과 중복)
 *                   ❌ 거래량≥130% 단발 (5일>20일이 더 정확)
 *          [D 안: 가중치 차등]
 *                 ADX 20 / MA20+기울기 15 / 52주 15 (= 50점, 핵심 추세)
 *                 ST 12 / MACD 12 (= 24점, 안심도)
 *                 거래량 10 / OBV 8 / RSI 8 (= 26점, 보조)
 *                 만점 100
 *          [영향]
 *                 - calcSustainedStrength 전체 재작성 (8 컴포넌트)
 *                 - 모든 탭의 강세 점수 자동 변경 (getSignalsV2가 호출)
 *                 - 강세 60+/80+ 임계값 그대로 (실험 후 조정 가능)
 *                 - 데이터 가용: ADX/PDI/MDI/OBV/RSI/MA20 모두 fetch_yahoo에 있음
 *          [최소 데이터] 20봉 필요 (이전 5봉 → 20봉, MA20/거래량 평균 계산용)
 *          [예상 효과]
 *                 - 횡보주 (KO 같은) 강세 점수 낮음 → 추천 안 됨
 *                 - 추세주 (NVDA/GOOGL 같은) 강세 점수 높음 → 추천 잘 됨
 *                 - 강세 60+ 통과 종목 수 감소 가능 (더 엄격해짐)
 *          [백업] App.v2.7.2.backup.jsx
...
function calcSustainedStrength(chartData) {
  ...
  // ② 가격>MA20 AND MA20 기울기 양수 (13점)
  let ma20Slope = 0;
  if (L >= 6 && last.ma20 != null) {
    const ma20_5ago = chartData[L - 6]?.ma20;
    if (ma20_5ago != null) ma20Slope = last.ma20 - ma20_5ago;
  }
  ...
  if (L >= 21) {
    ...
    const last20 = chartData.slice(-21, -1).map(c => c.volume || 0).filter(v => v > 0);
    if (last5.length === 5 && last20.length === 20) {
      const avg5 = last5.reduce((a, b) => a + b, 0) / 5;
      const avg20 = last20.reduce((a, b) => a + b, 0) / 20;
      volQuality = avg5 > avg20;
    }
  }
  if (volQuality) {
    score += 8;
  ...
  let ma5gt20 = false;
  if (L >= 5 && last.ma20 != null) {
    const last5closes = chartData.slice(-5).map(c => c.close).filter(c => c != null);
    if (last5closes.length === 5) {
      const ma5 = last5closes.reduce((a, b) => a + b, 0) / 5;
      ma5gt20 = ma5 > last.ma20;
    }
  }
  if (ma5gt20) {
    score += 3;
  ...
  if (L >= 6 && last.obv != null) {
    ...
    if (obv5ago != null && close5ago != null) {
      obvCohesion = (last.close > close5ago) && (last.obv > obv5ago);
    }
  }
  if (obvCohesion) {
    score += 4;
    breakdown.push({ label: "OBV 동행 (5일)", pts: 4, ok: true });
  } else {
  ...
  if (last.rsi != null && last.rsi >= 50 && last.rsi <= 70) {
    score += 4;
    breakdown.push({ label: "RSI 50~70", pts: 4, ok: true, note: `${last.rsi.toFixed(0)}` });
  } else {
        ...
        {tab==="lab"&&<div style={{padding:"12px 14px"}}>
          ...
          {(()=>{
              ...
              .map(([ticker,c])=>{
                ...
                const helper = (idx) => {
                  ...
                  return { idx, candle,
                    ...
                    // ★ v2.7.5: 신규 2개 컴포넌트 측정
                    c_ma5gt20: (() => {                                // 18. MA5 > MA20 (단기 모멘텀)
                      if (idx < 4 || candle.ma20 == null) return false;
                      const last5 = [d[idx-4], d[idx-3], d[idx-2], d[idx-1], candle]
                        .map(c => c?.close).filter(c => c != null);
                      if (last5.length !== 5) return false;
                      const ma5 = last5.reduce((a, b) => a + b, 0) / 5;
                      return ma5 > candle.ma20;
                    })(),
                    c_pdiPos: candle.pdi != null && candle.mdi != null && candle.pdi > candle.mdi,  // 19. +DI > -DI
                  };
                };
            ...
            const components = [
              ...
              // ★ v2.7.5: 신규 2개 추가 (검증 예정)
              {key:"c_ma5gt20",    label:"🆕 MA5 > MA20 (단기 모멘텀)", group:"강세", color:"#FF9F0A"},
              {key:"c_pdiPos",     label:"🆕 +DI > -DI (방향성)",    group:"강세", color:"#FF9F0A"},
            ];
