...
def send_buy_alerts(stocks_data, market_filter="all", intraday=False):
    ...
    for ticker, stock in stocks_data.items():
        ...
        try:
            ...
            if sig and sig["tier"] in ("confirmed_super", "confirmed_strong", "candidate_super", "candidate_strong"):
                candidates.append({"ticker": ticker, "stock": stock, "sig": sig, "key": key})
        except Exception:
            continue
    # 슈퍼 먼저, 강력 그 다음 — 정렬
    candidates.sort(key=lambda c: (
        0 if c["sig"]["tier"] in ("confirmed_super", "candidate_super") else 1,
        -c["sig"]["count"]
    ))
    ...
    if not candidates:
        ...
        return
    super_count = sum(1 for c in candidates if c["sig"]["tier"] in ("confirmed_super", "candidate_super"))
    ...
    if intraday:
        ...
        if super_count > 0:
            header += f"<b>🚀 슈퍼 {super_count}개</b> + 강력 후보 (거래량 장 마감 시 확정)\n\n"
        else:
            header += "<i>핵심 2+ 충족 후보 (거래량 미확정)</i>\n\n"
    else:
        header = f"🟢 <b>{market_label} 종가 확정 신호</b> — {datetime.datetime.now().strftime('%m/%d %H:%M')} KST\n"
        if super_count > 0:
            header += f"<b>🚀 슈퍼 {super_count}개</b> (핵심 3/3 충족) — 최고 등급\n<i>다음 거래일 시가 매수 검토</i>\n\n"
        else:
            header += "<i>핵심 2+ 충족 — 다음 거래일 시가 매수 검토</i>\n\n"
    body_lines = []
    for i, c in enumerate(candidates, 1):
        ticker = c["ticker"]; s = c["stock"]; sig = c["sig"]["filters"]
        tier = c["sig"]["tier"]
        is_super = tier in ("confirmed_super", "candidate_super")
...
# ══════════════════════════════════════════════════════════════
# 저장 전 캔들 다이어트 (2026-09)
#   stocks.json 이 11MB 였고 그중 98% 가 종목별 캔들 127봉이었습니다.
#   이 파일은 하루 두 번 통째로 커밋돼 저장소가 109MB 까지 불었습니다.
#   · 알파 터미널은 이 파일을 읽지 않습니다 (snapshot.json / bars 사용)
#   · 베타 터미널의 F-Score 는 RSI14·20일 박스·5일 변동만 씁니다 → 40봉이면 충분
#   계산은 수집 직후 전체 봉으로 이미 끝났고, 여기서는 '저장할 분량'만 줄입니다.
# ══════════════════════════════════════════════════════════════
KEEP_CANDLES = 40
...
def slim_candles(output):
    ...
    for sec in ("stocks", "pool"):
        for st in (output.get(sec) or {}).values():
            cd = st.get("candles")
            if isinstance(cd, list) and len(cd) > KEEP_CANDLES:
                cut += len(cd) - KEEP_CANDLES
                st["candles"] = cd[-KEEP_CANDLES:]
    if cut:
        print(f"  🧹 캔들 정리 — 저장 분량에서 {cut:,}봉 제외 (종목당 최근 {KEEP_CANDLES}봉 유지)")
    return output
