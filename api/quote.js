...
async function getKRQuote(ticker, token) {
  ...
  const data = await r.json();
  const o = data.output;
  if (!o || !o.stck_prpr) throw new Error("한국 종목 조회 실패");
...
async function getUSQuote(ticker, token) {
  ...
  if (!o || !o.last) {
    ...
    return buildUSResult(ticker, o2);
  }
  return buildUSResult(ticker, o);
}

function buildUSResult(ticker, o) {
  ...
  return {
    ticker,
    label: (o.rsym||ticker).replace(/^(NAS|NYS|AMS|BAQ)/,'').replace(/^[A-Z]{1,3}/,'').trim()||ticker,
    price,
    change,
    changePct,
    market: "🇺🇸",
